...
app.controller('ctrl',function($scope,$state,$http,$mdToast,$mdDialog){
  ...
  $scope.forgotPassword = function(data){
    console.log(data);
    $http.get('https://269af816.ngrok.io/forgotPassword/'+$scope.userId+'/'+data).then(function(data){
      console.log(data);
...
$scope.decryptPrompt = function(ev,file) {
  ...
  $scope.login = function(data){
    console.log("login");
    $http.post('https://269af816.ngrok.io/signinUser', data).then(function(data){
      if (data.data.success) {
  ...
  $scope.signup = function(data){
    $http.post('https://269af816.ngrok.io/registerUser', data).then(function(data){
      if (data.data.success) {
        $mdToast.show(
          $mdToast.simple()
  ...
  $scope.createFolder= function(data){
    console.log(data);
    $http.get('https://269af816.ngrok.io/createFolder/'+$scope.userId+'/'+data).then(function(data){
      if (data.data.success) {
        $mdToast.show(
          $mdToast.simple()
  ...
  $scope.getFolderNames= function(){
    $http.get('https://269af816.ngrok.io/getFolderNames/'+$scope.userId).then(function(data){
  ...
  $scope.loadFiles= function(){
    $http.get('https://269af816.ngrok.io/loadFiles/'+$scope.userId+'/'+$scope.selectedFolder).then(function(data){
      console.log(data);
      if (data.data.success) {
        if(!$scope.$$phase) {
  ...
  $scope.downloadFile = function(data,key){
    console.log(data);
    let filedata = data
    $http.get("https://269af816.ngrok.io/checkHash/"+$scope.userId+'/'+$scope.selectedFolder+'/'+data+'/'+key).then(function(data){
      console.log(data);
      if (data.data.valid) {
        window.location.assign(`https://269af816.ngrok.io/downloadFile/`+$scope.userId+'/'+$scope.selectedFolder+'/'+filedata+'/'+key);
      }else {
  ...
  $scope.deleteFile = function(data){
    console.log(data);
    $http.get(`https://269af816.ngrok.io/deleteFile/`+$scope.userId+'/'+$scope.selectedFolder+'/'+data).then(function(data){
      console.log(data);
      if (data.data.success) {
        $scope.loadFiles();
...
app.controller('MyCtrl',['Upload','$window',function(Upload,$window){
        ...
        vm.upload = function (file) {
          ...
          console.log(appScope.key);
            Upload.upload({
                url: 'https://269af816.ngrok.io/uploadFile/'+appScope.userId+'/'+appScope.selectedFolder+'/'+appScope.key, //webAPI exposed to upload the file
